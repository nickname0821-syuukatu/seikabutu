// ARAP + Fibonacci 2-hit LSRV junction-aware
...
// 高速 globalStep（Adjacency list + is_joint 版）
void globalStep(
...
// body 用 r-ring 構築（graph distance version）
...
// localStep_ARAP_RR with body r-ring smoothing
void localStep_ARAP_RR_integrated(
    const std::vector<Vec3>& V0,
    const std::vector<Vec3>& V,
    const std::vector<std::vector<Neighbor>>& nbr,
    const std::vector<char>& is_joint,
    const std::vector<std::vector<int>>& rrings, // ← 追加
    std::vector<Mat3>& R)                        // ← 最後の引数として R を受け取る
{
    std::vector<Mat3> Rprev = R; // ← 更新中 R を使わず固定
    const int n = V0.size();
    ...
    if(!is_joint[i]){
        // body の場合：r-ring の平均回転で平滑化
        for(int j : rrings[i]){
            Srr += Rprev[j];
        }
        double lambda_body = 0.8; // 強さ調整
        Stilde = Si + lambda_body * Srr;
    } else {
        // joint の場合は従来通り
        for(int j : rrings[i]){
            Srr += Rprev[j];
        }
        double lambda_joint = 0.2;
        Stilde = Si + lambda_joint * Srr; // rrings[i] = 1-ring
    }

    // SVD で回転を更新
    Eigen::JacobiSVD<Mat3> svd(
        Stilde, Eigen::ComputeFullU | Eigen::ComputeFullV);
    Mat3 U = svd.matrixU();
    Mat3 Vt = svd.matrixV().transpose();
    if((U*Vt).determinant() < 0.0) U.col(2) *= -1.0;
...
int main(int argc,char** argv)
{
    ...
    std::vector<int> handle_ids = {
        31469,
        99327,100480,106653,112002,113414,111234,105630,100471,
        37037,34504,30811,27391,25268,26308,29572,34497,
        59463,59466,59194,61057,107670,58712,56079};
    std::vector<Vec3> handle_pos = {V[31469]+Vec3(0,-20,20),
        Vec3(0.87,-12,4),Vec3(2,-11,4.1),Vec3(2.8,-10,4.8),Vec3(2.4,-8.8,5.6),Vec3(1.1,-8,6),Vec3(-0.46,-8.4,5.8),Vec3(-0.9,-10,4.9),Vec3(-0.23,-11,4.2),
        Vec3(V[25268][0],-30,14.1),Vec3(V[27391][0],-30,13.6),Vec3(V[30811][0],-30,11.9),Vec3(V[34504][0],-30,10),Vec3(V[37037][0],-30.8,8.9),Vec3(V[34497][0],-30,10.1),Vec3(V[29572][0],-30,11),Vec3(V[26308][0],-30,13),
        V[59463],V[59466],V[59194],V[61057],V[107670],V[58712],V[56079]
    };
